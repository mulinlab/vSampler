...
public final class JarCommadLine {
    ...
    private final static int HELP_SIMILARITY_FLOOR = GP.HELP_SIMILARITY_FLOOR;
    private final static int MINIMUM_SUBSTRING_LENGTH = GP.MINIMUM_SUBSTRING_LENGTH;
    ...
    public static void main(final String[] args) {
        System.exit(new JarCommadLine().instanceMain(args, getPackageList(), GP.PRO_CMD));
    }
    ...
    protected int instanceMain(final String[] args) {
        return instanceMain(args, getPackageList(), GP.PRO_CMD);
    }
    ...
    public static void processAllCommandLinePrograms(
        ...
        for (final Class clazz : classFinder.getClasses()) {
            if (!clazz.isInterface() && !clazz.isSynthetic() && !clazz.isPrimitive() && !clazz.isLocalClass()
                ...
                clpClassProcessor.accept(clazz, JarCommadLine.getProgramProperty(clazz));
            }
        }
    }
    ...
    public static void printUsage(final PrintStream destinationStream, final Set<Class<?>> classes, final String commandLineName) {
        ...
        for (final Map.Entry<CommandLineProgramGroup, List<Class<?>>> entry : programsByGroup.entrySet()) {
            ...
            Collections.sort(sortedClasses, new JarCommadLine.SimpleNameComparator());
